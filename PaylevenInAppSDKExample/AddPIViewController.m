...
@property (weak, nonatomic) IBOutlet UITextField *useCaseTextField;
...
- (void)viewDidLoad {
    ...
    [self.useCaseTextField addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
    ...
    if (self.userToken) {
        ...
        [[PLVInAppClient sharedInstance] addPaymentInstrument:newCreditCardPi
                                                 forUserToken:self.userToken
                                                   andUseCase:self.useCaseTextField.text
...
- (void)closeKeyboard {
    [self.useCaseTextField resignFirstResponder];
